...
def generate_org(input_file, input_org, filename, selected_user):
    ...
    # 1. First node: Organization node
    org_node = {
        "id": 0,
    ...
    # 2. List to collect all nodes
    nodes_list = [org_node]


    for (
        id_, pid, person_name, department_name, title, node_type,
        company_name, website, employee_range, industry, business_solution,
        address_1, zip_code, city, state, country, email_id,
        seniority_level, job_function, linkedin_url, public_profile,
        contact_number1, contact_number2, notes,
        edu1, edu2, edu3,
        past_company1, past_company2, past_company3, past_company4, past_company5,
        insight_heading1, insight_content1, insight_link1, insight_date1,
        insight_heading2,
        insight_content2,
        insight_link2,
        insight_date2,
    ...
    # nodes_json = json.dumps(nodes_list, indent=4, default=convert_numpy)
    ...
    # Replace the placeholder in the template with the generated content # commented
    ...
    # # # Write the modified content to the output HTML file # commented
    # with open(os.path.join(save_path,f"{filename}.html"), "w", encoding='utf-8') as output_file:
    #     output_file.write(final_content)
    #     output_file.close()
    return True, personCount, departmentNames, nodes_list
